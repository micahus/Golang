...
func main() {
	// 连接数据库
	db, err := gorm.Open(mysql.Open("micah:shijinping@(192.168.80.5:3306)/pandora?charset=utf8mb4&parseTime=True&loc=Local"))
	if err != nil {
	...
	//		gen.FieldRelate(field.HasMany, "Address", Address, &field.RelateConfig{GORMTag: map[string][]string{"UID": {"foreignKey"}}}),
	//	)...,
	//)
	//Address = g.GenerateModel("address",
	//	append(
	//		fieldOpts,
	//		gen.FieldRelate(field.BelongsTo, "User", User, &field.RelateConfig{GORMTag: map[string][]string{"UID": {"foreignKey"}}}),
	//	)...,
	//)
